...
def equivalent_realpath(p):
    ...
    try:
        ...
        return link if os.path.dirname(link) == pdir else p
    except Exception:
        return p
...
def list_commands(prefix, highlighted_reals, highlighted_dirs):
    for suffix in ["", "3", "3.8", "3.9", "3.10", "3.11", "3.12"]:
        ...
        if target is not None:
            ...
            line = f" - {cmd.ljust(9)} {relation} {real}"
            if (
...
def can_use_ansi_codes():
    ...
    try:
        ...
        warnings.warn(f"Can't determine Windows version {ver}")
        return False
...
def normpath_with_actual_case(name):
    ...
    assert os.path.isabs(name) or os.path.ismount(
        name
    ), f"Not abs nor mount: {name}"
    assert os.path.exists(name), f"Not exists: {name}"
...
if __name__ == "__main__":
    ...
    if sys_real[-9:-1] == "python3.":
        likely_pips.append(f"{sys_real[:-9]}pip3.{sys_real[-1]}")
